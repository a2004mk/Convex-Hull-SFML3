...
// конструктор: создание пол€ ввода с лейблом и настройками
InputField::InputField(const sf::Font& font, const std::string& fieldLabel,
    ...
    // курсор ввода (мигающа€ вертикальна€ черта)
    cursor.setSize({ 2, 20 });
    cursor.setFillColor(sf::Color::White);
    updateCursorPosition();
}

// обновление позиции курсора после изменени€ текста
...
void InputField::handleEvent(const sf::Event& event) {
    ...
    if (event.is<sf::Event::TextEntered>()) {
        ...
        if (isSelected && textEntered) {
            // backspace Ч удаление последнего символа
            ...
            // допустимые символы: цифры, точка, минус
                ...
                if (isdigit(c) || c == '.' || c == '-') {
                    // первый символ после фокуса Ч замен€ем "0"
                    if (justSelected) {
                        input.clear();
                        justSelected = false;
                    }
            ...
            updateCursorPosition();
            cursorClock.restart();  // сброс таймера мигани€
            showCursor = true;
        }
    }
    // клик мыши Ч выбор пол€ дл€ ввода
    ...
    // наведение мыши Ч подсветка рамки
...
// установка значени€ из кода (например, при открытии редактора)
...
// получение числового значени€ из строки ввода
float InputField::getValue() const {
    ...
    try {
        ...
        return 0;  // невалидный ввод Ч возвращаем 0
    }
}
...
// отрисовка пол€: фон, лейбл, значение, курсор
void InputField::draw(sf::RenderWindow& window, const sf::Font& font) {
    ...
    // курсор рисуем только если поле активно и сейчас его "видима€" фаза
    if (showCursor && isSelected) {
        window.draw(cursor);
    }
}

// сброс пол€ к начальному значению "0"
